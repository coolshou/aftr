...
AddPortMapping(struct upnphttp * h, const char * action)
{
	...
	r = upnp_redirect(eport, int_ip, iport, protocol, lifetime, desc);
...
GetSpecificPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	r = upnp_get_redirection_infos(eport, protocol);
	...
	else
	{
		syslog(LOG_INFO, "%s: rhost='%s' %s %s found => %s:%u desc='%s'",
		       action,
		       r_host, ext_port, protocol, r->iaddr, r->iport, r->desc);
		bodylen = snprintf(body, sizeof(body), resp,
				action, "urn:schemas-upnp-org:service:WANIPConnection:1",
				r->iport, r->iaddr, r->desc,
				(unsigned int)r->lifetime, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	}
...
GetGenericPortMappingEntry(struct upnphttp * h, const char * action)
{
	...
	r = upnp_get_redirection_infos_by_index(index);
	...
	else
	{
		...
		bodylen = snprintf(body, sizeof(body), resp,
			action, "urn:schemas-upnp-org:service:WANIPConnection:1",
			r->eport, r->proto, r->iport, r->iaddr, r->desc,
			(unsigned int)r->lifetime, action);
		BuildSendAndCloseSoapResp(h, body, bodylen);
	}
